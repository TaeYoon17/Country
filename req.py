...
def oneFile(url,num):
    ...
    ParticipaintList=list(map(makeTextlist,soup.find("table",{"id":"datatable-PART_P_OC"}).find_all("td",{"data-title":"Country/Territory"})))
    ObserveList=list(map(makeTextlist,soup.find("table",{"id":"datatable-PART_O_OC"}).find_all("td",{"data-title":"Country/Territory"})))
    ...
    for i in range(0,max(oLen,pLen)):
        ...
        if i>=oLen:
            ObserveList.append("")
        print(ParticipaintList[i],ObserveList[i])
